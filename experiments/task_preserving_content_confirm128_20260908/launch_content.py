...
def main():
    ...
    for model,gpu in [('qwen7',6),('qwen3vl8',7)]:
        ...
        if model=='qwen3vl8': env['PYTHONPATH']='/path/to/local/cta_crossvl_env/lib/python3.10/site-packages'
        cmd=['/path/to/local/.venv/bin/python',str(code/'run_content.py'),'run','--root',str(root),'--base',str(base),'--model',model]
